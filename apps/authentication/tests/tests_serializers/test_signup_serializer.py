...
class SignupSerializerTests(TestCase):
    ...
    def test_signup_fields_is_required(self):
        ...
        self.assertEqual(str(serializer.errors['firstName'][0]), error_messages('required',  'Prénom'))
        self.assertEqual(str(serializer.errors['lastName'][0]), error_messages('required',  'Nom'))
        self.assertEqual(str(serializer.errors['email'][0]), error_messages('required',  'email'))
        self.assertEqual(str(serializer.errors['password'][0]), error_messages('required',  'Mot de passe'))
        self.assertEqual(str(serializer.errors['confirmPassword'][0]), error_messages('required',  'Confirmation mot de passe'))
    ...
    def test_signup_firstName_field_is_not_blank(self):
        ...
        self.assertFalse(serializer.is_valid())
        self.assertEqual(set(serializer.errors), set(['firstName', 'lastName', 'email', 'password', 'confirmPassword']))
        self.assertEqual(str(serializer.errors['firstName'][0]), error_messages('blank',  'Prénom'))
        self.assertEqual(str(serializer.errors['lastName'][0]), error_messages('blank',  'Nom'))
        self.assertEqual(str(serializer.errors['email'][0]), error_messages('blank',  'email'))
        self.assertEqual(str(serializer.errors['password'][0]), error_messages('blank',  'Mot de passe'))
        self.assertEqual(str(serializer.errors['confirmPassword'][0]), error_messages('blank',  'Confirmation mot de passe'))
